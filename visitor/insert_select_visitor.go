...
type InsertSelectVisitor struct {
...
func NewInsertSelectVisitor(ctx *dal_context.DalContext) *InsertSelectVisitor {
	return &InsertSelectVisitor{
...
// 计算语句有几个
func (this *InsertSelectVisitor) incrStmtNo() {
...
func (this *InsertSelectVisitor) popStmtNo() {
	this.CurrVisitorStmt = nil
...
// 记录当前语句块
func (this *InsertSelectVisitor) setCurrBlock(currBlock int) {
	this.BlockHeap.Push(this.CurrBlock) // 将当前block保存下来
	this.CurrBlock = currBlock          // 重置当前block
}

// pop语句块
func (this *InsertSelectVisitor) popBlock() {
	if block, ok := this.BlockHeap.Pop(); ok {
		this.CurrBlock = block
	}
}

func (this *InsertSelectVisitor) Enter(in ast.Node) (out ast.Node, skipChildren bool) {
	// 有错误直接退出
	if this.Err != nil {
		return in, true
	}
...
func (this *InsertSelectVisitor) Leave(in ast.Node) (out ast.Node, ok bool) {
	defer func() {
		this.CurrNodeLevel--
	}()
	...
	switch node := in.(type) {
	case *ast.InsertStmt:
		this.Err = this.leaveInsertStmt(node)
	case *ast.SelectStmt:
...
// 进入 insert 节点
func (this *InsertSelectVisitor) enterInsertStmt(node *ast.InsertStmt) error {
	// 设置当前所在的语句块
	this.setCurrBlock(BLOCK_INSERT_INTO)
...
func (this *InsertSelectVisitor) leaveInsertStmt(node *ast.InsertStmt) error {
	// 退出当前块
	defer this.popBlock()
	// pop 出语句号, 并计算应该到第几个语句了
...
func (this *InsertSelectVisitor) enterSelectStmt(node *ast.SelectStmt) error {
	// 计算语句号, 计算语句有几个
	this.incrStmtNo()
	return nil
}
...
func (this *InsertSelectVisitor) leaveSelectStmt(node *ast.SelectStmt) error {
	// pop 出语句号, 并计算应该到第几个语句了
	defer this.popStmtNo()
...
func (this *InsertSelectVisitor) enterFieldList(node *ast.FieldList) error {
...
// 离开 Select Field 节点
func (this *InsertSelectVisitor) leaveFieldList(node *ast.FieldList) error {
	defer this.popBlock()
...
// 进入 TableSource
func (this *InsertSelectVisitor) enterTableSource(node *ast.TableSource) error {
	// 判断该表是否是分表
...
// 离开 TableSource
func (this *InsertSelectVisitor) leaveTableSource(node *ast.TableSource) error {
	// 当前语句没有分表信息, 直接退出
	if this.CurrVisitorStmt == nil {
		return nil
	}
...
func (this *InsertSelectVisitor) enterBinaryOperationExpr(node *ast.BinaryOperationExpr) error {
...
func (this *InsertSelectVisitor) leaveBinaryOperationExpr(node *ast.BinaryOperationExpr) error {
	defer this.popBlock()
...
// 进入 IN 语句
func (this *InsertSelectVisitor) enterPatternInExpr(node *ast.PatternInExpr) error {
	this.setCurrBlock(BLOCK_PATTERN_IN)
	return nil
}

// 离开 IN 语句
func (this *InsertSelectVisitor) leavePatternInExpr(node *ast.PatternInExpr) error {
	defer this.popBlock()
	if this.CurrVisitorStmt == nil { // 如果该语句中没有分库分表, 就不用管了
		return nil
	}
...
// 进入 Like 语句
func (this *InsertSelectVisitor) enterPatternLikeExpr(node *ast.PatternLikeExpr) error {
...
// 离开 Like 语句
func (this *InsertSelectVisitor) leavePatternLikeExpr(node *ast.PatternLikeExpr) error {
	defer this.popBlock()
	if this.CurrVisitorStmt == nil { // 如果该语句中没有分库分表, 就不用管了
		return nil
	}
...
// 进入 BETWEEN 语句
func (this *InsertSelectVisitor) enterBetweenExpr(node *ast.BetweenExpr) error {
	this.setCurrBlock(BLOCK_BETWEEN)
	return nil
}

// 离开 BETWEEN 语句
func (this *InsertSelectVisitor) leaveBetweenExpr(node *ast.BetweenExpr) error {
	defer this.popBlock()
	if this.CurrVisitorStmt == nil { // 如果该语句中没有分库分表, 就不用管了
		return nil
	}
